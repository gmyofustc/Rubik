...
def _libnccl():
    ...
    if lib is not None:
        ...
        lib = ctypes.cdll.LoadLibrary('path-to-nccl.so')
...
def is_available(tensors):
    ...
    if _libnccl() is None:
        warnings.warn("NCCL library not found, Check LD_LIBRARY_PATH")
        return False
    return True
...
def broadcast(input, root, stream=None):
    comm = communicator()
    if stream is not None:
        stream = stream.cuda.stream
    data_type = nccl_types[input.type()]
    return check_error(lib.ncclBcast(
        ctypes.c_void_p(input.data.ptr()),
        ctypes.c_size_t(input.numel()),
        data_type,
        ctypes.c_size_t(root),
        comm,
        ctypes.c_void_p(stream)
    ))

def all_gather(input, stream=None):
    #TODO